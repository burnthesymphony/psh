 ...
 function list_(){
  ...
  //die("NANAN");
    ...
    // $this->load->view('detail_payroll',$data);
	 ...
	 $this->load->view('new_detail_payroll',$data);
  ...
  $ci= &get_instance();
    $result = $ci->global_model->get_salary(array('id_karyawan'=>$id_karyawan,'bulan'=>$bulan,'tahun'=>$tahun,'b.jenis'=>$jenis));
    if($jenis=='TUNJANGAN')
      $label='Earning';
  ...
  function get_absensi($bulan_payroll,$tahun_payroll,$id_karyawan){
     ...
     $ci= &get_instance();
	$absensi= $ci->global_model->get_data(array('table'=>'trs_absensi_karyawan',
